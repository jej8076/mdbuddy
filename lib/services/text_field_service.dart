...
class _LineStyledTextFieldState extends State<LineStyledTextField> {
  ...
  Offset _getCursorOffset() {
    ...
    // 현재 줄의 스타일 가져오기
    final lineStyle = widget.lineStyles[currentLineIndex % widget.lineStyles.length];
    ...
    return Offset(
        widget.padding.left + textPainter.width,
        yOffset
    );
  }
  ...
  void _handleCharacterInput(String character) {
    final text = widget.controller.text;
    final newText = text.substring(0, _cursorPosition) + character +
        text.substring(_cursorPosition);
    widget.controller.text = newText;
    setState(() {
      _cursorPosition++;
